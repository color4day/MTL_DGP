...
class FirstLayer(PyroModule):
    ...
    def __init__(
        ...
        #pyro.sample()
        if 'init_1st_w' in kwargs:
            init_w = kwargs['init_1st_w']
            # print("init_w", init_w.mean.shape, init_w.var.shape)
            self.layer.weight =  PyroSample(dist.Normal(init_w.mean, init_w.var).expand([self.J, in_dim]).to_event(2))
            print("1st layer w initialization success!", np.shape(self.layer.weight))
...
class SecondLayer(PyroModule):
    ...
    def __init__(
        ...
        if 'init_2nd_w' in kwargs:
            init_w = kwargs['init_2nd_w']
            # print("init_w", init_w.mean.shape, init_w.var.shape)
            self.layer.weight =  PyroSample(dist.Normal(init_w.mean, init_w.var).expand([out_dim, hid_dim]).to_event(2))
            print("2nd layer w initialization success!", np.shape(self.layer.weight))

        # self.layer.bias = pyro.sample(f"{num_layer}-th bias",dist.Normal(0., torch.tensor(1.0,device = 'cuda')).expand([out_dim]).to_event(1))
        self.layer.bias = PyroSample(dist.Normal(0., torch.tensor(1.0, device='cuda')).expand([out_dim]).to_event(1))
        if 'init_2nd_b' in kwargs:
            init_b = kwargs['init_2nd_b']
            self.layer.bias =  PyroSample(dist.Normal(init_b.mean, init_b.var).expand([out_dim]).to_event(1))
            print("2nd layer b initialization success!", np.shape(self.layer.bias))
    # known: phi(Omega x)
    # want: phi(Omega x) W + epsilon
    # epsilon : (2,)
    # Secondlayer:
